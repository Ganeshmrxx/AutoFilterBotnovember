...
API_ID = int(environ.get('API_ID', '24330185')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', 'd024679a644eca8a88d10feaf4537d96')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "8099791335:AAHBseTEVRX6gxHxzA6PPTfWSJvwU0KpwwQ")    # Bot token from @BotFather
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002199758641 -1003034120038').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001942141488'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1001942141488'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1001942141488'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1001942141488').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001942141488')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001942141488')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', '-1001942141488')  # Support group link (make sure bot is admin)

# FORCE_SUB 
auth_req_channels = environ.get("AUTH_REQ_CHANNELS", "-1001942141488")# requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
auth_channels     = environ.get("AUTH_CHANNELS", "-1001942141488")# Channels for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Ttt:00000000.mm@ttt.rtzavkc.mongodb.net/?appName=Ttt")  # MongoDB URI for the database
...
TMDB_API_KEY = environ.get('TMDB_API_KEY', '30360a0a5775cc28c00c5a3cedc047bf') # preffer to use your own tmdb API Key get it from https://www.themoviedb.org/settings/api
...
PORT = int(environ.get("PORT", "9090"))  # Port for the web server (default: 8080)
